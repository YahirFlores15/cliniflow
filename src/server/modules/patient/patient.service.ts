import { cancelPatientAppointment, checkPatientAppointmentAvailability, findPatientAppointmentById, findPatientDoctorSchedule, findPatientProfileByUserId, reschedulePatientAppointment, updatePatientProfile, } from "@/server/modules/patient/patient.repository";
...
import type { PatientAppointmentAvailabilityReason, PatientAppointmentDTO, PatientProfileDTO, } from "@/shared/dtos/patient.dtos";
import { getDb, } from "@/server/db/connection";


const MINIMUM_APPOINTMENT_NOTICE_HOURS =
    8;

export class PatientDomainError
    extends Error {
    constructor(
        message: string
    ) {
...
function getAvailabilityErrorMessage(
    reason:
        PatientAppointmentAvailabilityReason
): string {
    switch (
    reason
    ) {
...
function requireOwnedAppointment(
    params: {
...
export function updateProfileForPatient(
    params: {
        userId: string;
        input:
        PatientUpdateProfileInput;
    }
): PatientProfileDTO {
    const profile =
    ...
    return updatePatientProfile({
        ...
        address:
            params.input.address.trim(),
    });
}

export function cancelAppointmentForPatient(
    params: {
        userId: string;
        input:
        PatientCancelAppointmentInput;
    }
): PatientAppointmentDTO {
    const profile =
        requirePatientProfile(
...
export function rescheduleAppointmentForPatient(
    params: {
        userId: string;
        input:
        PatientRescheduleAppointmentInput;
    }
): PatientAppointmentDTO {
    const profile =
        requirePatientProfile(
